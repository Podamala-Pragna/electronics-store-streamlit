...
with st.expander("Filters", expanded=True):
    ...
    category = f3.selectbox("Category", ["All"] + store.list_categories(), key="filter_cat")
...
if is_staff(user):
    with st.expander("Add Product", expanded=False):
        ...
        category_in = st.selectbox("Category", store.list_categories(), key="add_cat")
        ...
        featured = st.checkbox("Mark as Featured", key="add_featured")
        ...
        if st.button("Save", key="add_save"):
            ...
            pid = store.add_product({
                "name": name, "brand": brand_in, "category": category_in,
                "condition": condition_in, "price": price, "stock": stock,
                "warranty_months": warranty, "desc": desc, "image_path": img_path,
                "created_by": user["email"], "featured": featured
            })
...
# -------------------- PRODUCTS LIST --------------------
if df.empty:
    st.info("No products yet. Staff can add products using the panel above.")
else:
